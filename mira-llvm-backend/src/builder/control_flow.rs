...
impl<'ctx, 'arena> FunctionCodegenContext<'ctx, 'arena, '_, '_, '_> {
    ...
    fn build_if_else(
        ...
        let [if_bb, else_bb, end_bb] = self.make_bb(["then", "else", "endif"]);
        self.build_conditional_branch(self.basic_value(cond).into_int_value(), if_bb, else_bb)?;

        self.goto(if_bb);
        self.build_block(if_block, scope, true)?;
        self.terminate(|| self.build_unconditional_branch(end_bb))?;
    ...
    pub(super) fn build_if(
        ...
        let [if_bb, end_bb] = self.make_bb(["then", "endif"]);
        self.build_conditional_branch(self.basic_value(cond).into_int_value(), if_bb, end_bb)?;
    ...
    pub(super) fn build_while(
        ...
        let [cond_bb, body_bb, end_bb] = self.make_bb(["while-cond", "while-body", "while-end"]);

        self.build_unconditional_branch(cond_bb)?;

        self.goto(cond_bb);
        self.build_block(cond_block, scope, false)?;
        self.terminate(|| {
            self.build_conditional_branch(self.basic_value(cond).into_int_value(), body_bb, end_bb)
        })?;

        self.goto(body_bb);
        self.build_block(body, scope, true)?;
        self.terminate(|| self.build_unconditional_branch(cond_bb))?;
