...
pub enum TypecheckingError<'arena> {
    ...
    UnsizedForSizedGeneric(Location, Type<'arena>),
    ...
    UnsizedReturnType(Location, Type<'arena>),
    #[error("{0}: Unsized Type {1} is not a valid argument")]
    UnsizedArgument(Location, Type<'arena>),
    #[error("{0}: Function {1} on trait {2} is not valid for &dyn {2} types")]
    InvalidDynTypeFunc(Location, InternedStr<'arena>, InternedStr<'arena>),
    #[error("{0}: Cannot find trait {1}")]
    CannotFindTrait(Location, PathWithoutGenerics<'arena>),
    ...
    NonSizedType(Location, Type<'arena>),
    #[error("{0}: Intrinsics accepts only integers, supplied: {1}")]
    IntOnlyIntrinsic(Location, Type<'arena>),
    #[error("{0}: Assembly instructions can only accept numeric types (i_, u_, f_ and bool). Specified Type: `{1}`")]
    AsmNonNumericTypeResolved(Location, Type<'arena>),
    #[error("{0}: Assembly instructions can only accept numeric types (i_, u_, f_ and bool). Specified Type: `{1}`")]
    AsmNonNumericType(Location, InternedStr<'arena>),
    ...
    NonMemberFunction(Location, InternedStr<'arena>, Type<'arena>),
    #[error("{0}: Cannot find function `{1}` on type `{2}`")]
    CannotFindFunctionOnType(Location, InternedStr<'arena>, Type<'arena>),
    #[error("{0}: Cannot find value `{1}`")]
    CannotFindValue(Location, Path<'arena>),
    #[error("{0}: Tried to access a member of a non-struct value `{1}`")]
    AccessNonStructValue(Location, Type<'arena>),
    #[error("{0}: Tried to index into non-array type `{1}`")]
    IndexNonArrayElem(Location, Type<'arena>),
    #[error("{0}: Could not find field `{2}` on `{1}`")]
    FieldNotFound(Location, Type<'arena>, InternedStr<'arena>),
    #[error("{0}: Invalid cast (lhs: `{1}`, rhs: `{2}`)")]
    DisallowedCast(Location, Type<'arena>, Type<'arena>),
    #[error("{0}: Cannot assign to this expression")]
    CannotAssign(Location),
    #[error("{0}: Cannot shift by a non-uint value (found `{1}`)")]
    CannotShiftByNonUInt(Location, Type<'arena>),
    #[error("{0}: Cannot add `{1}`")]
    CannotAdd(Location, Type<'arena>),
    #[error("{0}: Cannot subtract `{1}`")]
    CannotSub(Location, Type<'arena>),
    #[error("{0}: Cannot multiply `{1}`")]
    CannotMul(Location, Type<'arena>),
    #[error("{0}: Cannot divide `{1}`")]
    CannotDiv(Location, Type<'arena>),
    #[error("{0}: Cannot take the remainder of `{1}`")]
    CannotMod(Location, Type<'arena>),
    #[error("{0}: Cannot binary and `{1}`")]
    CannotBAnd(Location, Type<'arena>),
    #[error("{0}: Cannot binary or `{1}`")]
    CannotBOr(Location, Type<'arena>),
    #[error("{0}: Cannot xor `{1}`")]
    CannotBXor(Location, Type<'arena>),
    #[error("{0}: Cannot and `{1}`")]
    CannotLAnd(Location, Type<'arena>),
    #[error("{0}: Cannot or `{1}`")]
    CannotLOr(Location, Type<'arena>),
    #[error("{0}: Cannot compare `{1}`")]
    CannotCompare(Location, Type<'arena>),
    #[error("{0}: Cannot compare the equality of `{1}`")]
    CannotEq(Location, Type<'arena>),
    #[error("{0}: Cannot shift `{1}` left")]
    CannotShl(Location, Type<'arena>),
    #[error("{0}: Cannot shift `{1}` right")]
    CannotShr(Location, Type<'arena>),
    #[error("{0}: Expected both sides of the expression to be the same, lhs: `{1}`, rhs: `{2}`")]
    LhsNotRhs(Location, Type<'arena>, Type<'arena>),
    #[error("{0}: Cannot negate a `{1}`")]
    CannotNeg(Location, Type<'arena>),
    #[error("{0}: Cannot use unary `+` on a `{1}`")]
    CannotPos(Location, Type<'arena>),
    #[error("{0}: Cannot not a `{1}`")]
    CannotLNot(Location, Type<'arena>),
    #[error("{0}: Cannot invert a `{1}`")]
    CannotBNot(Location, Type<'arena>),
    #[error("{0}: cannot dereference a `{1}`")]
    CannotDeref(Location, Type<'arena>),
    ...
    MismatchingTraits(Location, Type<'arena>, Vec<InternedStr<'arena>>),
